...
# B. Outlier Handling (Capping Expenditure at 99th Percentile)
exp_cap = odisha_df['Household_usual_consumer_expendi'].quantile(0.99)
odisha_df['Household_usual_consumer_expendi_clean'] = np.where(
    odisha_df['Household_usual_consumer_expendi'] > exp_cap,
    exp_cap,
    odisha_df['Household_usual_consumer_expendi']
)

# C. Variable Creation: Economic Status (Quartiles)
odisha_df['Economic_Status'] = pd.qcut(
    odisha_df['Household_usual_consumer_expendi_clean'], 
    q=4, 
    labels=['Poorest', 'Lower Middle', 'Upper Middle', 'Richest']
)

# D. Variable Creation: Sector (Rural vs Urban)
odisha_df['Sector_Label'] = odisha_df['Sector'].map({1: 'Rural', 2: 'Urban'})

# E. Variable Creation: Household Size Bins (Defining "Family Size")
...
# F. Variable Creation: Social Group Label
social_map = {1: 'ST', 2: 'SC', 3: 'OBC', 9: 'Others'}
odisha_df['Social_Group_Label'] = odisha_df['Social_group'].map(social_map).fillna('Other')

# G. Variable Creation: Source of Finance Label
...
# Table 5: Source of Finance (Cross-tabulation)
table5 = pd.crosstab(
    sick_households['Economic_Status'], 
    sick_households['Finance_Label'], 
    normalize='index'
) * 100
...
with open(output_file, 'w') as f:
    ...
    f.write(table1.round(2).to_string())
    f.write("\n\n")
    
    f.write("--- TABLE 2: ANALYSIS BY SECTOR (RURAL/URBAN) ---\n")
    f.write(table2.round(2).to_string())
    f.write("\n\n")
    
    f.write("--- TABLE 3: ANALYSIS BY SOCIAL GROUP ---\n")
    f.write(table3.round(2).to_string())
    f.write("\n\n")
    
    f.write("--- TABLE 4: ANALYSIS BY FAMILY SIZE ---\n")
    f.write(table4.round(2).to_string())
    f.write("\n\n")
    
    f.write("--- TABLE 5: SOURCE OF FINANCE BY ECONOMIC STATUS (%) ---\n")
    f.write("(Rows sum to 100%)\n")
    f.write(table5.round(2).to_string())
    f.write("\n\n")
...
sns.barplot(data=odisha_df, x='Economic_Status', y='Has_Ailment', hue='Sector_Label', 
...
plt.figure(figsize=(10, 6))
# Use the table data calculated above for consistency
...
# Graph 3: Medical Expenditure Distribution (Boxplot)
plt.figure(figsize=(10, 6))
spenders = sick_households[sick_households['outpatient_expenditure_total_Rs'] > 0]
sns.boxplot(data=spenders, x='Economic_Status', y='outpatient_expenditure_total_Rs', 
            palette="Set2", hue='Economic_Status', legend=False)
plt.yscale('log')
...
# Graph 4: Prevalence by Social Group
plt.figure(figsize=(10, 6))
sns.barplot(data=odisha_df, x='Social_Group_Label', y='Has_Ailment', 
            palette="viridis", estimator=np.mean, errorbar=None, 
            hue='Social_Group_Label', legend=False)
...
plt.figure(figsize=(10, 6))
sns.barplot(data=odisha_df, x='Family_Size_Bin', y='Has_Ailment', 
            palette="coolwarm", estimator=np.mean, errorbar=None, 
            hue='Family_Size_Bin', legend=False)
